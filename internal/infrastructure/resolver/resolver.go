...
func (r *ReferenceResolver) ResolveAll(ctx context.Context, data map[string]interface{}, basePath string, config domain.Config) error {
	...
	// Remove self-referencing components (e.g., {$ref: "#/components/schemas/X"} in schemas.X)
	for _, ct := range componentTypes {
		if section, ok := components[ct].(map[string]interface{}); ok {
			for name, component := range section {
				if componentMap, ok := component.(map[string]interface{}); ok {
					if refVal, hasRef := componentMap["$ref"]; hasRef && len(componentMap) == 1 {
						if refStr, ok := refVal.(string); ok {
							if refStr == "#/components/"+ct+"/"+name {
								delete(section, name)
							}
						}
					}
	...
	// Merge collected components into the main components section
	for _, ct := range componentTypes {
		...
		for name, component := range r.components[ct] {
			...
			// Skip if component with same hash already exists
			componentHash := r.hashComponent(component)
			if existingName, exists := r.componentHashes[componentHash]; exists && existingName != normalizedName {
				continue
			}
			
			// Add or replace component
			if _, exists := section[normalizedName]; !exists {
				section[normalizedName] = component
				r.componentHashes[componentHash] = normalizedName
			} else if !r.componentsEqual(section[normalizedName], component) {
				// Component exists but is different - create unique name
